...
describe("reduce method", () => {
  ...
  it("groups by genre", () => {
    ...
    const groupedShows = shows.reduce((obj, cur) => {
      if (!obj[cur.genre]) obj[cur.genre] = [];
      obj[cur.genre].push(cur.title);
      return obj;
    }, {});
  ...
  it("groups by key (2)", () => {
    // TODO: do something with `shows` here
    const groupedShows = shows.reduce((obj, cur) => {
      const index = obj.findIndex((_obj) => _obj.genre === cur.genre);
      if (index === -1) obj.push({ genre: cur.genre, titles: [cur.title] });
      else obj[index].titles.push(cur.title);
      return obj;
    }, []);
    expect(groupedShows).toEqual([
      {
  ...
  it("extracts writer names", () => {
    // TODO: do something with `shows` here
    const writerNames = shows.reduce((acc, cur) => {
      acc.push(...cur.writers);
      return acc;
    }, []);
